...
module TB_chip_top_mid_pSquare_dSHARES_IPM_kRED_verilog;
...
wire clk_PAD, rst_PAD, read_PAD, write_PAD;
wire [2:0] address_PAD;
...
// Unit Under Test
chip_top UUT (.clk_PAD(clk_PAD), .rst_PAD(rst_PAD), .read_PAD(read_PAD), .write_PAD(write_PAD), .address_PAD(address_PAD), .data_in_PAD(data_in_PAD), .data_out_PAD(data_out));
...
assign clk_PAD = clk;
assign rst_PAD = rst;
assign read_PAD = read;
assign write_PAD = write;
assign address_PAD = address;
